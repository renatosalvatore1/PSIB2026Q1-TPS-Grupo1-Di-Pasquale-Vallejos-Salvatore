...
Al elegir el sujeto, se carga el archivo y aparecen pestañas:
   ...
   - PSD: periodograma de Welch del canal/ventana seleccionado.
...
class TabEEG(QtWidgets.QWidget):
    def __init__(self, raw, parent=None):
        ...
        self.dur_combo.addItems(["10 s", "30 s", "60 s"])
...
class TabPSD(QtWidgets.QWidget):
    def __init__(self, raw, parent=None):
        ...
        self.dur_combo = QtWidgets.QComboBox()
        self.dur_combo.addItems(["10 s", "30 s", "60 s", "Todo el registro"])
        self.dur_combo.setCurrentText("30 s")
    ...
    def duracion(self):
        txt = self.dur_combo.currentText()
        if txt == "Todo el registro":
            return -1
    ...
    def graficar(self):
        ...
        if dur == -1:
            data = self.raw.get_data(picks=canal)
            senal = data[0]
            titulo_ventana = "registro completo"
        else:
            t0 = self.slider.value()
            t1 = min(t0 + dur, self.dur_total)
            self.label_tiempo.setText(f"{t0:.0f} s")
            data, _ = self.raw.copy().pick([canal]).get_data(tmin=t0, tmax=t1, return_times=True)
            senal = data[0]
            titulo_ventana = f"[{t0:.0f}-{t1:.0f}] s"
        ...
        ax.set_title(f"PSD — canal {canal} | {titulo_ventana}")
        ax.grid(True, alpha=0.3)
